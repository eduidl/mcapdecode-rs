...
impl McpClient {
    fn start(allow_root: &Path) -> Self {
        Self::start_with_protocol(allow_root, "2025-06-18")
    }

    fn start_with_protocol(allow_root: &Path, protocol_version: &str) -> Self {
...
fn mcp_discovers_tools_and_inspects_topics_and_schemas() {
    ...
    assert_eq!(
        client.initialization()["result"]["serverInfo"]["name"],
        "mcapq"
    );
    assert_eq!(
        client.initialization()["result"]["serverInfo"]["version"],
        env!("CARGO_PKG_VERSION")
    );
    assert!(
        client.initialization()["result"]["instructions"]
            .as_str()
            .unwrap()
            .contains(fixture.parent().unwrap().to_str().unwrap())
    );
...
#[test]
fn mcp_legacy_protocol_receives_text_content_fallback() {
    let fixture = demo_fixture();
    let mut client = McpClient::start_with_protocol(fixture.parent().unwrap(), "2024-11-05");
    assert_eq!(
        client.initialization()["result"]["protocolVersion"],
        "2024-11-05"
    );

    let response = client.tool("mcap_info", json!({"path": fixture}));
    let result = &response["result"];
...
fn mcp_rereads_changed_files_and_rejects_unsafe_requests() {
    ...
    assert!(outside_error.contains("outside the allowed roots"));
    assert!(outside_error.contains(directory.path().to_str().unwrap()));

    let outside_directory = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let directory_error = client.tool_error("mcap_info", json!({"path": outside_directory}));
    assert!(directory_error.contains("outside the allowed roots"));

    let inside_directory = directory.path().join("nested");
    fs::create_dir(&inside_directory).unwrap();
    let inside_directory_error = client.tool_error("mcap_info", json!({"path": inside_directory}));
    assert!(inside_directory_error.contains("is not a file"));

    let missing_inside = directory.path().join("typo.mcap");
    let missing_inside_error = client.tool_error("mcap_info", json!({"path": missing_inside}));
    assert!(missing_inside_error.contains("failed to resolve MCAP path"));
    ...
    #[cfg(unix)]
    {
        ...
        assert!(escaped_error.contains("outside the allowed roots"));

        let escaped_dir = directory.path().join("escaped-dir");
        symlink(demo_fixture().parent().unwrap(), &escaped_dir).unwrap();
        let escaped_through_dir = escaped_dir.join(demo_fixture().file_name().unwrap());
        let escaped_dir_error =
            client.tool_error("mcap_info", json!({"path": escaped_through_dir}));
        assert!(escaped_dir_error.contains("outside the allowed roots"));
    }
